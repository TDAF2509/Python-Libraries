...
BlackRun=[pygame.image.load('BlackRun1.png'),pygame.image.load('BlackRun2.png'),pygame.image.load('BlackRun3.png'),pygame.image.load('BlackRun4.png'),pygame.image.load('BlackRun5.png'),pygame.image.load('BlackRun6.png'),pygame.image.load('BlackRun7.png'),pygame.image.load('BlackRun8.png'),pygame.image.load('BlackRun9.png')]
...
bg7=pygame.image.load('Forrestbg.png')
...
BgTXT=pygame.image.load('BgTEXT.png')
...
bg7=pygame.transform.scale(bg7,(screenWidth+screenWidth+screenWidth+screenWidth+screenWidth-48,screenHeight))
bgChange=[bg1,bg2,bg3,bg4,bg5,bg6,bg7]
CharacterRun=[SonicRun,BlackRun]
CharacterJump=[Jump,BlackJump]
CharacterSlide=[Jump,BlackSlide]
###########INITIALISING##############################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################
gameX=300
gameY=380
playerposX=300
...
###########BACKGROUND ANIMATION AND SETTINGS######################################################################################################################################################################################                    
    def BackgroundChange0():
        ...
        window.blit(bgChange[4],[n,0])
        n=n-15
        window.blit(bgChange[4],[screenWidth+screenWidth+screenWidth+screenWidth+screenWidth+n,0])
        n=n-50
...
###########CHARACTER ANIMATION#######################################################################################################################################################################################
    def redrawGameWindow():
        ...
        if Run:
            ...
            walkCount +=1
        if isJump:
            window.blit(BlackJump[i],(gameX,gameY))
        if Slide:
            window.blit(BlackSlide[j],(gameX,gameY))
        pygame.display.update()
###########GAME BUILD UP######################################################################################################################################################################################
    gameStart=True
    gameRun=False
...
def SHOP_MENU():
    ...
    while Shop:
        ...
        mouse=pygame.mouse.get_pos()
        click=pygame.mouse.get_pressed()
        if mouse[0]>=410 and mouse[0]<=775 and mouse[1]>=200 and mouse[1]<=450:
            pygame.draw.rect(window,Lgold,[410,200,365,250])
            window.blit(BgTXT,(455,300))
            if click[0]==1:
                print("BACKGROUND SHOP")
        else:
            pygame.draw.rect(window,gold,[410,200,365,250])
            window.blit(BgTXT,(455,300))
        
        if mouse[0]>=15 and mouse[0]<=390 and mouse[1]>=200 and mouse[1]<=450:
            pygame.draw.rect(window,Lgreen,[15,200,375,250])
            window.blit(CharTXT,(80,300))
            if click[0]==1:
                print("CHARACTER SHOP")
        else:
            pygame.draw.rect(window,green,[15,200,375,250])
            window.blit(CharTXT,(80,300))
...
def OldUser():
    ...
    while Username not in AccountNames[i] and i < total:
        ...
        while Password not in AccountPasswords[i]:
            ...
            print("Login successful please raise the mouse!")
            CurrentScore=0
            Stats=open(Username+".txt","r")
